                      ...
                      <form id="productform" >
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control" id="logo"
                                    aria-describedby="emailHelp" name="logo" value="{{$logo?? ''}}" placeholder="Enter Logo" >
                            </div>

                            <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Desperation</label>
                                <textarea class="form-control" id="desperation" name="desperation" placeholder="Enter Location">{{$desperation ?? ''}}</textarea>
                            </div>
                            <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Location</label>
                                <input type="text" class="form-control" id="location"
                                    aria-describedby="emailHelp" name="location" value="{{$location ?? ''}}" placeholder="Enter Location" >
                            </div>
                             <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Phone</label>
                                <input type="text" class="form-control" id="phone"
                                    aria-describedby="emailHelp" name="phone" value="{{$phone ?? ''}}" placeholder="Enter Phone" >
                            </div>
                            <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Email</label>
                                <input type="text" class="form-control" id="email"
                                    aria-describedby="emailHelp" name="email" value="{{$email ?? ''}}" placeholder="Enter Email" >
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="text" name="text" placeholder="Enter Text">{{$text ?? ''}}</textarea>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#productform').on('submit', function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route('admin.footer.manage_process') }}',
                data: formData,
                processData: false,
                contentType: false,
                headers: {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href = "{{ route('admin.footer.list') }}";
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
